...
- **无看门狗依赖**：TermWrap.dll 运行时动态适配 `termsrv.dll`，不需要 INI 或定时任务；服务崩溃由系统 SCM（FailureActions）自动恢复。
...
| 2 | 会话设置（最大并发、每用户单会话） |
| 3 | 安全设置（NLA、安全层） |
| 4 | 远程控制 / 影子模式（本机 + 跨机远程；console 会话也可影） |
...
选择菜单「0」或运行 `.\termwrap.ps1 -Uninstall`：恢复安装前的 `ServiceDll`、删除已部署的 DLL、移除 Defender 排除项，并还原音频重定向配置。
...
- 本工具属于自维护分支，与 rdpwarp（`rdpwarps.ps1`）等其他 wrapper **互斥**：同一服务指针同一时刻只能指向一个包装 DLL。安装前会检测占用，命中则拒绝并提示（可加 `-Force` 强制覆盖）。
- 状态保存在 `%ProgramData%\termwrap`；旧版本位于 `%ProgramData%\rdpwarp` 的状态文件会在启动时自动迁移。
