...
function Deploy-TermWrapBinaries {
    ...
    try {
        ...
        if ((Get-RegDword $REG_RDP fDenyTSConnections) -ne 0) {
            Write-I "Remote Desktop 未启用（fDenyTSConnections=1），正在启用 RDP..."
            $rdpPort = (Get-ItemProperty -Path $REG_RDP_WS -Name PortNumber -ErrorAction SilentlyContinue).PortNumber
            if (-not $rdpPort) { $rdpPort = 3389 }
            Enable-RdpHostAccess -Port $rdpPort
            if ((Get-RegDword $REG_RDP fDenyTSConnections) -ne 0) { throw 'Failed to enable Remote Desktop' }
            Write-S "RDP 已启用（端口 $rdpPort，含 TCP/UDP 防火墙入站规则）"
        }
